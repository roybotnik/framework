...
router.prototype = {
	...
	dispatch : function (context) {
		...
		if (route === null) {
			this.logger.log('No route found for ' + context.req.url);
		...
		this.logger.log("Calling -> Controller: " + route.controller + ", Action: " + route.action);

		// Run the controller method that is specified in the route
		this.controllers[route.controller][route.action](context); 
	},
